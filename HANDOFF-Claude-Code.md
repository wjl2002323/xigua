...
## 实现偏差记录（2026-07-23 code-review 轮）
- Link 字段：原型用缩写 `{ s, d, km, deal, dealAt? }`；后端请用 spec 全名
  `{ supplyId, demandId, km, deal, dealAt? }`。`dealAt` 在用户点击"标记已成交"时写入
  ISO 时间戳；种子预置的历史成交无时间戳。
- 半径默认值 60/200/420 指后端"用户未填时的缺省值"；mock 生成的是分布区间
  40-90 / 100-260 / 260-620，两者不矛盾。
- 手机布局激活：除 `?mobile=1` 外，实现增加了 ≤640px 视口自动切换（原 spec 未要求，
  属可用性增强）。
- 临期红色脉冲：spec 文字将其列为点位基础编码，设计原稿实现仅在"紧迫度"视图生效。
  以设计原稿为准保留现状，是否改为全局待产品确认。
- 资产本地化（国内部署前提）：主屏 d3 / topojson-client / world-atlas 地图拓扑已
  vendor 进 `vendor/`，Google Fonts 移除（serif 栈退化到本机宋体族）；省界 DataV
  GeoAtlas 为国内源保留网络加载（失败降级已有）。手机三屏（dc-runtime）仍在运行时
  从 unpkg 拉 React/Babel —— 属设计画布工件，未本地化。
